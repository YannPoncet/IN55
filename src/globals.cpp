...
const int WINDOW_HEIGHT = 1280; // Base height of the window (if not maximized)
const int WINDOW_WIDTH = 720; // Base width of the window (if not maximized)

bool showBezier = false; // If we should show the bezier curve
bool showSystem = false; // If we should show the coordinates system
bool showSoil = true; // If we should show the soil underneath the morel
bool showLights = false; // If we should show the lights cubes

QVector<bool> lightsEnabled = {0,0,0,0,1}; // Only light on camera is enabled by default
...
Parameters parameters = {
    ...
    0,          // choosenSet -> is set in GeometryEngine::createAndBindMorels, go there to change it
